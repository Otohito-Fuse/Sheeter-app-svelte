...
export class Chord {
    ...
    constructor(noteSymbol: NoteSymbol | null, accidental: Accidental | null, suffix1: string, suffix2: string, bassNoteSymbol: NoteSymbol | null, bassAccidental: Accidental | null) {
...
export function parseChord(s: string): ChordPlaceHolder | null {
    ...
    if (s.match(/^[A-G](|b|bb|#|##)(|(_[^_\^\/]*))(|(\^[^_\^\/]*))(|(\/[A-G](|b|bb|#|##)))$/)) {
        ...
        let noteSymbol: NoteSymbol | null = constructNoteSymbol(rootString.charAt(0));
        let accidental: Accidental | null = constructAccidental(rootString.substring(1));
        let suffix1: string = !!vecUnderbar[1] ? vecUnderbar[1] : "";
        let suffix2: string = !!vecCaret[1] ? vecCaret[1] : "";
        let bassNoteSymbol: NoteSymbol | null = !!vecSlash[1] ? constructNoteSymbol(vecSlash[1].charAt(0)) : null;
        let bassAccidental: Accidental | null = !!vecSlash[1] ? constructAccidental(vecSlash[1].substring(1)) : null;
        return new Chord(noteSymbol, accidental, suffix1, suffix2, bassNoteSymbol, bassAccidental);
    }
    if (s.match(/^\/[A-G](|b|bb|#|##)$/)) {
        let bassNoteSymbol: NoteSymbol | null = constructNoteSymbol(s.charAt(1));
        let bassAccidental: Accidental | null = constructAccidental(s.substring(2));
...
export function parseChords(s: string): Array<ChordInBar> {
    ...
    let chordsRaw: Array<ChordPlaceHolder> = chordStrings.map(parseChord).filter((x) => (!!x));
    let n: number = chordsRaw.length;
    let calcXRel = (i: number) => (barPaddingLeftRatio + (1 - barPaddingLeftRatio - barPaddingRightRatio) * (i / n));
