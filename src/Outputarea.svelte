<script lang="ts">
    ...
    import { viewBoxHeight, viewBoxWidth, yFirstStave, yStaveInterval } from './config/lengths';
    ...
    {#each staves as stave, i}
        ...
        {#each stave.bars as bar, j}
            {#each bar.noteHeads as { xRel, yRel, noteHeadType, dots, accidental }}
                <NoteHeadComponent {noteHeadType} {xRel} {yRel} {dots} {accidental} {i} {j} numOfBars={stave.bars.length} numOfKeySignatures={Math.abs(stave.keySignature)} timeSignatureNumer={stave.timeSignatureNumer} timeSignatureDenom={stave.timeSignatureDenom} />
            {/each}
            {#each bar.rests as { xRel, restType, dots }}
                <RestComponent {restType} {xRel} {dots} {i} {j} rests={bar.rests} numOfBars={stave.bars.length} numOfKeySignatures={Math.abs(stave.keySignature)} timeSignatureNumer={stave.timeSignatureNumer} timeSignatureDenom={stave.timeSignatureDenom} />
            {/each}
            <TiesComponent {i} {j} ties={bar.ties} numOfBars={stave.bars.length} numOfKeySignatures={Math.abs(stave.keySignature)} timeSignatureNumer={stave.timeSignatureNumer} timeSignatureDenom={stave.timeSignatureDenom} />
        {/each}
        {#each stave.chords as chords, j}
            {#each chords as { chordPlaceHolder, xRel }}
                <ChordComponent {xRel} {i} {j} {chordPlaceHolder} staveBarsLength={stave.bars.length} numOfKeySignatures={Math.abs(stave.keySignature)} timeSignatureNumer={stave.timeSignatureNumer} timeSignatureDenom={stave.timeSignatureDenom} />
            {/each}
        {/each}
    {/each}
</svg>
